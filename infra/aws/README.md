...
| `outputs.tf`   | Everything the Helm release, CI, and KEDA need |
...
Use the shared chart at
[`deploy/charts/family-media-bot`](../../deploy/charts/family-media-bot) with
[`deploy/values/aws.yaml`](../../deploy/values/aws.yaml). Install KEDA and
cluster-autoscaler with the values under
[`deploy/addons/aws/`](../../deploy/addons/aws/), then follow the chart
[deployment guide](../../deploy/charts/family-media-bot/README.md). The files
under `deploy/legacy/` are historical and must not be applied beside Helm.
